...
篇幅所限，结论背后的展开叙述不在此详述，读者请参阅文章《为何软件不应有主》于 <https://gnu.org/philosophy/why-free.html>，以及[《如今自由软件更加重要》](free-software-even-more-important.md)。
...
开发一个系统是个非常大的工程。为了达成目标，我决定尽可能适配并使用现有的自由软件。例如，一开始我就决定用 <!--(pdf)\TeX\begin{comment} (pdf)-->TeX<!--(pdf) \end{comment}(pdf)--> 来做主要的文本排版器；若干年后，我决定使用 X 窗口系统而不是为 GNU 再写一个窗口系统。
...
### 著左权（Copyleft）和 GNU GPL 

GNU 的目标是给与用户自由，而不仅仅是流行。所以我们需要使用可以阻止 GNU 软件被转变成私有软件的发行条款。我们使用的方法叫“著左权” [^thegnu-5]。

著左权使用版权法，但使它的作用与之通常的作用相反：它成为一种保持程序自由的手段，而不是限制程序的手段。

著左权的核心思想是给与任何人运行程序、复制程序、改写程序，和发行改写后的程序的许可——但那些人无权添加自己的限制。这样一来，定义“自由软件”的关键自由藉由每个拥有副本的人得以保证，这些自由成了不可剥夺的权利。

对于一个有效的著左权，修改版也得是自由的。这确保了建立在我们的工作基础上的作品发布后将有用于社区。当以程序员为业者志愿改进 GNU 软件时，著左权能防止他们的雇主说：“你不能分享那些改进，因为我们要用它们来做一个我们的私有版本。”
...
一个有关的问题涉及将自由的程序和非自由的代码结合到一起。这样的结合体将不可避免变得不自由；任何一个在非自由部分上缺失的自由也将在整体上缺失。允许这样的结合将会打开足以沉掉一艘船的缺口：任何添加或结合到有著左权程序上的东西必须使得更大的结合版也是自由和有著左权的。

我们明确为大多数 GNU 软件使用了有著左权的 GNU 通用公共许可证，或简称 GNU GPL。我们在特定场合下有其它种类的著左权可以使用。GNU 手册也是有著左权的，但使用一个非常简化的著左权类型，因为 GNU GPL 的复杂性对手册是不需要的[^thegnu-6]。
...
FSF 的大部分收入曾经来自销售自由软件的副本和其他相关服务（源代码的 CD-ROM、二进制文件的 CD-ROM、精心印刷的手册，都有着再发行和修改的自由），以及豪华的发行版（我们为客户选择的平台定制的完整的软件集）。今天 FSF 仍然销售手册和其他部件 [^thegnu-7]，但大部分的资金来自成员的会费。你可以通过 <https://fsf.org/join> 来加入 FSF。
...
GNU C 库使用一种特别的著左权，称作 GNU 库 GPL（LGPL）[^thegnu-10]，允许将私有软件链接到该库上。为什么需要这样的特例？
...
[^thegnu-5]: 在 1984 或 1985 年，Don Hopkins（一位非常有想象力的朋友）寄给我一封信。在信封上他写下了不少有趣的话，包括这句：“Copyleft——逆转一切权利。”我用 “copyleft” 一词来命名我正在开发的发行理念。（原译文采用了贾星克、李极光发表在云南师范大学学报的《论自由软件运动》一文中对 copyleft 一词的译法，即“左版”。编辑本则改用 GNU 中文版网站“哲学”专区“短文和长篇”目录中 GNU CTT 留下的一个微妙译法，即“著左权”。——编者注）
