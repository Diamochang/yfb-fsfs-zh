...
在电路和非装饰性的物体外观上的著左权（Copyleft），不会像想象一般获得成功。这些设计的版权仅适用于这些设计是如何被画出或写下的。著左权是一种利用版权法的手段，所以其效果仅限于版权法起效的范围。

例如，一个电路，作为一种拓扑关系，无法拥有版权（也因此无法使用著左权）。由 HDL 写成的电路定义可以拥有版权（因此可以使用著左权），但著左权只能保护这 HDL 代码的具体表达方式，而非依其生成的电路拓扑。与此类似，电路的图纸或布局可以拥有版权，因此可以使用著左权，但仅仅保护图纸或布局本身，而非电路拓扑。任何人都可以以不同的外观画出同样的电路拓扑，或写出生成相同电路的不同 HDL 定义。
...
我们已经有了可以让我们的硬件设计自由的合适许可证。我们需要的是作为一个社区将其认作我们所必做之事而在我们自行装配物品时坚持自由的设计。
 ...
 [^freehard-12]: 参见[《还在用“知识产权”这词吗？它只是看上去很美》](not-ipr.md)一文。 
